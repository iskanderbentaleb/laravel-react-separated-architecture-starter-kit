...
export default function UserSettings() {
  ...
  const handleToggleTwoFactor = useCallback(
    async (enabled) => {
      ...
      try {
        const { data } = await adminApi.toggleTwoFactor(enabled);
        setUser(data.user);
        setUserData(data.user);
  ...
  const handleConfirmTwoFactor = async () => {
    ...
    try {
      const { data } = await adminApi.verifyTwoFactor({
  ...
  const handleCancelTwoFactorSetup = async () => {
    ...
    try {
      const { data } = await adminApi.toggleTwoFactor(false);
      setUser(data.user);
      setUserData(data.user);
      setTwoFactorModalOpened(false);
      setTwoFactorCode('');
  ...
  const handleLogoutAllDevices = async () => {
    ...
    try {
      await adminApi.logoutOtherDevices();
